...
int main() {
    ...
    // load formula lib
    void *lib_handle = dlopen("./mandelbrot.so", RTLD_NOW);
    if (NULL == lib_handle)
    {
        fprintf(stderr, "Error loading mandelbrot formula: %s", dlerror());
        return -1;
    }
    pf_obj *(*pfn)(void);
    ...
    if (NULL == pfn)
    {
        fprintf(stderr, "Error loading mandelbrot formula symbols: %s", dlerror());
        dlclose(lib_handle);
        return -1;
    }
